		...
		try {
			...
			while (userHP > 0 && oppoHP > 0) { // eslint-disable-line no-unmodified-loop-condition
				...
				if (choice === 'saldır') {
					...
					await message.channel.send(`${user}, kendisini savundu!`);
					guard = true;
					reset(false);
					...
					if (!miss) {
						...
						await message.channel.send(`${user}, Ultra Güç Kullandın ve **${damage}** hasar vurdun!!`);
						dealDamage(damage);
					} else {
						await message.channel.send(`${user}, Enerjin Yeterli Olmadığı İçin Ultra Güç Kullanamadın`);
					}
					reset();
				} else if (choice === 'kaç') {
